...
test('手动触发：按钮 → POST → 处理中 → 完成后出现音源切换，切增强请求带 variant', async ({
  ...
  // 点击触发 → POST 恰好一次 → 进入处理中态
  await enhanceBtn.click();
  await expect(page.getByTestId('enhance-processing')).toBeVisible({ timeout: 10_000 });
  expect(triggerCount).toBe(1);

  // 轮询推进到 completed → 「原声/增强」切换出现（轮询间隔 5s，放宽等待）
  const enhancedTab = page.getByTestId('audio-enhanced');
  await expect(enhancedTab).toBeVisible({ timeout: 15_000 });
...
test('admin 设置：音频增强 tab 渲染配置项，测试连接展示 worker 探测结果', async ({
  ...
  await expect(page.getByPlaceholder('https://enhance.example.com')).toBeVisible({
    timeout: 10_000,
  });
