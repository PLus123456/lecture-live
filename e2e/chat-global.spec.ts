...
test('桌面端：模型选择器移到对话侧栏底部（不再在 composer 工具行内）', async ({
  page,
}) => {
  ...
  // 任务8：模型按钮应恰好出现一次（桌面端 composer 内不再渲染 → 只剩侧栏那一个）
  const modelBtn = page.getByRole('button', { name: 'Mock Model' });
  await expect(modelBtn).toHaveCount(1);
  await expect(modelBtn).toBeVisible();
  // 且这唯一的模型按钮位于对话侧栏（aside）内 —— 证明确实是「移到侧栏」而非仍在 composer
  await expect(
    page.locator('aside').getByRole('button', { name: 'Mock Model' })
  ).toBeVisible();
  // 侧栏底部「模型/Model」小标题（e2e 默认英文 locale）
  await expect(page.getByText(/^(模型|Model)$/).first()).toBeVisible();
  await snap(page, 'u15-model-in-sidebar');
});
