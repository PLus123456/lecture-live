...
function installSessionApiMocks(page: Page, opts: MockOptions = {}) {
  ...
  // 终态）。静态 PAUSED 会触发 playback 页的非终态弹回守卫（playback/page.tsx），表现为
  // 「跳到回放又瞬间弹回录音页」——此前用例全靠 toHaveURL 轮询恰好命中弹回前的窗口才通过。
  let finalized = false;
  ...
  return page.route('**/api/**', async (route: Route) => {
    ...
    if (p === `/api/sessions/${SESSION_ID}` && method === 'GET') {
      ...
      return fulfillJson(route, {
        id: SESSION_ID,
        title: 'Reclaim E2E',
        status: finalized ? 'COMPLETED' : sessionStatus,
        sourceLang: 'en',
        targetLang: 'zh',
      });
    }
...
test('正常停止：finalize 成功(无 alreadyCompleted)才清空本地缓存并跳回放', async ({
  ...
  // Next dev 按需编译 + NAS 慢 I/O：playback 路由首编 >15s。停止后的客户端导航要等
  // RSC 编译完成 URL 才变，冷 server 下 toHaveURL 必超时（表现为停在会话页）。
  // 用 page.request 并行预热编译（不经 page.route mock、不动页面状态）；上限同步放宽。
  test.setTimeout(60_000);
  const playbackWarmup = page.request
    .get(`/session/${SESSION_ID}/playback`)
    .catch(() => undefined);
  ...
  // 且停稳在回放页：曾因 mock 静态返回 PAUSED 被 playback 守卫瞬间弹回录音页，
  // 上面的轮询偶尔恰好命中弹回前窗口而假绿。确认不再被弹回。
