  ...
  it('创建会话时校验文件夹归属并规范化字段', async () => {
    ...
    expect(sessionCreateMock).toHaveBeenCalledWith({
      data: expect.objectContaining({
        ...
        // 用户显式起名且未声明自动占位 → 标记为 false（收尾后台不会覆盖）
        titleAutoGenerated: false,
      }),
    });
  });

  it('titleAutoGenerated：客户端声明或未传标题时标记为占位', async () => {
    sessionCreateMock.mockResolvedValue({
      id: 'session-2',
      title: '会话 7月12日 15:58',
      status: 'CREATED',
    });

    // 声明 titleAutoGenerated: true（NewSessionModal 的自动日期占位）
    await POST(
      createJsonRequest('http://localhost:3000/api/sessions', {
        method: 'POST',
        body: { title: '会话 7月12日 15:58', titleAutoGenerated: true },
      }),
      {} as never
    );
    expect(sessionCreateMock).toHaveBeenLastCalledWith({
      data: expect.objectContaining({ titleAutoGenerated: true }),
    });

    // 完全没传标题 → 'Untitled Session' 兜底也是占位
    await POST(
      createJsonRequest('http://localhost:3000/api/sessions', {
        method: 'POST',
        body: {},
    ...
    // 非 boolean true 的脏值不生效
    await POST(
      createJsonRequest('http://localhost:3000/api/sessions', {
        method: 'POST',
        body: { title: '我的课', titleAutoGenerated: 'yes' },
      }),
      {} as never
    );
    expect(sessionCreateMock).toHaveBeenLastCalledWith({
      data: expect.objectContaining({ titleAutoGenerated: false }),
    });
  });
