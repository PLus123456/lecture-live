...
export const POST = withRequestLogging('sessions:create', async (req: Request) => {
  ...
  try {
    ...
    // 客户端声明「标题是自动占位」（如「会话 {日期}」）；收尾后台据此决定是否自动生成正式标题。
    // 只信 boolean true；未传标题时用的 'Untitled Session' 兜底也属于占位。
    const titleAutoGenerated =
      body.titleAutoGenerated === true || body.title === undefined;
