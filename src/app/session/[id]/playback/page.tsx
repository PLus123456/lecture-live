...
export default function PlaybackPage() {
  ...
  // 收尾后台生成（总摘要报告 + 自动标题）监控：finalize 返回时二者仍在后台 LLM 任务里
  // 生成（常见 30s~2min），完成后在此原地刷新报告与标题，免手动刷新整页。
  const [reportPendingBg, setReportPendingBg] = useState(false);
  const bgRefreshAbortRef = useRef<AbortController | null>(null);
  const bgRefreshStartedRef = useRef(false);
  const reportDataRef = useRef<SessionReportData | null>(null);
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      createdUrls.forEach((url) => revokeAccountObjectUrl(url));
  ...
  // ── 收尾后台生成结果的自动刷新（总摘要报告 + 自动标题） ──
  //
  // finalize 接口在报告/标题生成完成**之前**就返回并跳转到本页（二者由服务端后台 LLM
  // 任务生成，常见 30s~2min 后才落库）。此前本页只在挂载时拉一次数据：标题一直停在
  // 「会话 {日期}」占位、报告 tab 空白，必须手动刷新整页才能看到。这里轮询后台任务队列，
  // 对应任务离开队列即拉取结果原地更新。终止条件：全部拿到 / 连续 3 轮队列中无本会话的
  // 生成任务（生成失败或被组策略跳过）/ 达到轮询上限。
  const startFinalizeBgRefresh = useCallback(
    (options: { optimistic: boolean }) => {
      if (!token || !sessionId) return;
      bgRefreshAbortRef.current?.abort();
      const ctrl = new AbortController();
      bgRefreshAbortRef.current = ctrl;

      const authHeaders: Record<string, string> = {
        Authorization: `Bearer ${token}`,
      };
      const needsReport = () =>
        !isSessionReportGenerationResolved(reportDataRef.current);
      const needsTitle = () => {
        const s = sessionRef.current;
        return !!s && isAutoGeneratedSessionTitle(s.title, s.titleAutoGenerated);
      };

      // 刚收尾跳转而来时报告几乎必然正在生成：先亮「生成中」，而不是闪一下「暂无报告」。
      if (options.optimistic && needsReport()) {
        setReportPendingBg(true);
      }

      // 各自返回该项是否已就绪（拿到数据 / 确认不再变化）。
      const refetchReport = async (): Promise<boolean> => {
        try {
          const res = await fetch(`/api/llm/report?sessionId=${sessionId}`, {
            headers: authHeaders,
            signal: ctrl.signal,
          });
          if (!res.ok) return false;
          const data = await res.json();
          if (isSessionReportGenerationResolved(data.report)) {
      ...
      const refetchTitle = async (): Promise<boolean> => {
        try {
          ...
          if (!res.ok) return false;
          const data = await res.json();
          if (!data || data.error || typeof data.title !== 'string') {
            return false;
          }
          setSession((prev) =>
            prev
              ? {
                  ...
                  titleAutoGenerated: data.titleAutoGenerated,
                }
              : prev
          );
          return !isAutoGeneratedSessionTitle(data.title, data.titleAutoGenerated);
        } catch {
          /* 网络抖动：下一轮再试 */
        }
        return false;
      ...
      let emptyTicks = 0;
      let ticks = 0;
      const MAX_TICKS = 48; // 5s × 48 ≈ 4 分钟兜底上限

      const tick = async () => {
        if (ctrl.signal.aborted) return;
        ticks += 1;

        let reportJobActive = false;
        let titleJobActive = false;
        ...
        try {
          ...
          if (res.ok) {
            const data = await res.json();
            queueChecked = true;
            const jobs: Array<{ sessionId?: string | null; type?: string }> =
              Array.isArray(data.jobs) ? data.jobs : [];
            for (const job of jobs) {
              if (job.sessionId !== sessionId) continue;
              if (job.type === 'report_generation') reportJobActive = true;
              if (job.type === 'title_generation') titleJobActive = true;
            }
          }
        } catch {
          /* 网络抖动：本轮不判定，直接排下一轮 */
        ...
        if (reportJobActive && needsReport()) {
          setReportPendingBg(true);
        }

        // active 时也探测已发布结果：JobQueue 成功终态写入失败会保守保留 activeKey，
        // 但版本化报告已可安全读取，不能让客户端空等到四分钟超时。
        let reportSettled = !needsReport();
        if (!reportSettled && queueChecked) {
          reportSettled = await refetchReport();
          // 标题任务已在跑而报告任务不在队列且报告拉不到：报告步骤已确定结束
          //（组禁用总摘要被跳过 / 生成失败），不再乐观显示「生成中」。
          if (!reportSettled && !reportJobActive && titleJobActive) {
            setReportPendingBg(false);
          }
        }
        let titleSettled = !needsTitle();
        if (!titleSettled && queueChecked && !titleJobActive) {
          titleSettled = await refetchTitle();
        }
        if (ctrl.signal.aborted) return;

        if (reportSettled && titleSettled) {
          setReportPendingBg(false);
          return;
        }

        if (reportJobActive || titleJobActive) {
          emptyTicks = 0;
        } else if (queueChecked) {
          emptyTicks += 1;
        }

        if (emptyTicks >= 3 || ticks >= MAX_TICKS) {
          setReportPendingBg(false);
          return;
        }
  ...
  const handleRegenerateReport = useCallback(async () => {
    ...
    try {
      ...
      if (res.status === 202) {
        // 同一报告版本已由 finalize/另一个标签页取得单飞 claim。不要把 202 当完成；
        // 复用后台任务轮询，等 winner 落盘后再刷新完整报告。
        setReportPendingBg(true);
        bgRefreshStartedRef.current = true;
        startFinalizeBgRefresh({ optimistic: true });
        return;
      }
      if (!res.ok) {
    ...
    regeneratingReport,
    t,
    startFinalizeBgRefresh,
  ]);

  // 启动条件：认证模式、初始加载完成、会话 COMPLETED，且报告缺失或标题仍是自动占位。
  // ?finalized=1 表示刚从录音收尾流程跳转而来，乐观点亮「报告生成中」。
  useEffect(() => {
    if (isShareMode || loading || !session) return;
    if (session.status !== 'COMPLETED') return;
    if (bgRefreshStartedRef.current) return;
    const needsReport = !isSessionReportGenerationResolved(
      reportDataRef.current
    );
    const needsTitle = isAutoGeneratedSessionTitle(
      session.title,
      session.titleAutoGenerated
    );
    if (!needsReport && !needsTitle) return;
    bgRefreshStartedRef.current = true;
    startFinalizeBgRefresh({
      optimistic: searchParams.get('finalized') === '1',
    });
  }, [isShareMode, loading, session, startFinalizeBgRefresh, searchParams]);
              ...
              {activeTab === 'report' && (
                <div className="space-y-4">
                  {reportLoading ? (
                    ...
                    <div
                      className="text-center text-charcoal-400 text-sm py-8"
