...
async function buildPptx(): Promise<Buffer> {
  ...
  zip.file(
    ...
    '<?xml version="1.0"?><p:sld xmlns:p="http://schemas.openxmlformats.org/presentationml/2006/main" ' +
      ...
      '<p:sp><p:txBody><a:bodyPr/><a:lstStyle/><a:p><a:r><a:t>Hello PPTX</a:t></a:r></a:p>' +
...
describe('fileExtractor', () => {
  ...
  describe('document parsing (xlsx / pdf)', () => {
    it('extracts text from a real OOXML DOCX in the child process', async () => {
    ...
    it('extracts text from a real OOXML PPTX in the child process', async () => {
      const result = await extractTextFromBuffer(
        await buildPptx(),
        'application/vnd.openxmlformats-officedocument.presentationml.presentation'
      );
      expect(result.text).toContain('Hello PPTX');
      expect(result.truncated).toBe(false);
    });
