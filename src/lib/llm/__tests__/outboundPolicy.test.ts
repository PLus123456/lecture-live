...
describe('LLM outbound SSRF policy', () => {
  it('canonicalizes default ports, a DNS root dot and a trailing slash', async () => {
    await expect(
      validateLlmProviderBaseUrl('https://api.example.com.:443/v1/', {
        lookupAddresses: publicDns,
      })
    ).resolves.toBe('https://api.example.com/v1');
  });

  it('keeps HTTPS mandatory outside development', async () => {
    await expect(
      validateLlmProviderBaseUrl('http://api.example.com/v1', {
        lookupAddresses: publicDns,
      })
    ).rejects.toThrow(/must use HTTPS/);
  });

  it('supports a public IPv6 origin with an explicit port', async () => {
    await expect(
      validateLlmProviderBaseUrl('https://[2001:4860:4860::8888]:8443/v1/')
  ...
  it.each([
    ...
    await expect(
      validateLlmProviderBaseUrl(url, { lookupAddresses: publicDns })
  ...
  it('rejects localhost and internal DNS suffixes before resolution', async () => {
    for (const host of ['localhost', 'llm.internal', 'provider.local', 'llm.home.arpa']) {
      await expect(
        validateLlmProviderBaseUrl(`https://${host}/v1`, {
          lookupAddresses: publicDns,
        })
  ...
  it('rejects a public-looking hostname if any DNS answer is private', async () => {
    ...
    await expect(
      validateLlmProviderBaseUrl('https://api.example.com/v1', {
  ...
  it('permits HTTP only behind the explicit non-production development switch', async () => {
    ...
    await expect(
      validateLlmProviderBaseUrl('http://8.8.8.8:8080/v1')
