...
export async function finalizeSession(
  ...
  let stagedTranscriptArtifacts: {
    transcript: StagedArtifact;
    summary: StagedArtifact;
  } | null = null;
  let stagedArtifactsCommitted = false;
  ...
  try {
    ...
    let finalDurationMs = clampSessionDurationMs(
      resolvedDurationMs,
      session.user.role
    );
    // 若本路径发布草稿录音，Session.durationMs 会改用媒体实测值；计费仍保留现有安全下限
    //（Soniox grant / 转录 / server 口径），避免上传一段极短录音冲掉已发生的串流成本。
    let billingDurationMs = finalDurationMs;
    ...
    if (!recordingPath) {
      ...
      if (merged) {
        ...
        try {
          measuredDurationMs =
            await measureAuthoritativeRecordingDurationMsFromBuffer(
              merged.buffer,
              merged.manifest.mimeType
            );
        } catch (error) {
          if (error instanceof RecordingDurationMeasurementError) {
            throw new FinalizeSessionError(422, {
        ...
        const maxDurationMs = getMaxSessionDurationMs(session.user.role);
        if (
          maxDurationMs !== null &&
          measuredDurationMs >
            maxDurationMs + RECORDING_DURATION_LIMIT_GRACE_MS
        ) {
          throw new FinalizeSessionError(422, {
            error: 'Recording exceeds the maximum session duration',
          });
        }
        finalDurationMs = measuredDurationMs;
        billingDurationMs = Math.max(
          measuredDurationMs,
          clampSessionDurationMs(resolvedDurationMs, session.user.role)
        );
    ...
    const billableMinutes = getBillableMinutes(billingDurationMs);

    if (!transcriptPath) {
      stagedTranscriptArtifacts = await stageSessionTranscriptArtifacts(
        session,
        bundle ?? EMPTY_TRANSCRIPT_BUNDLE
      );
      transcriptPath = stagedTranscriptArtifacts.transcript.reference;
      summaryPath = stagedTranscriptArtifacts.summary.reference;
    ...
    const commit = await prisma.$transaction(async (tx) => {
      ...
      if (shouldDeduct) {
        ...
        await deductTranscriptionMinutes(session.userId, billableMinutes, tx, {
          ...
          referenceId: options.sessionId,
        });
      }

      const stagedPublications = await settleStagedArtifactsInTransaction(tx, [
        ...(stagedAudio ? [stagedAudio] : []),
        ...(stagedTranscriptArtifacts
          ? [
              stagedTranscriptArtifacts.transcript,
              stagedTranscriptArtifacts.summary,
            ]
          : []),
      ]);
      ...
      return { lockLost: false as const, updated, stagedPublications };
    });
    ...
    if (commit.lockLost) {
      ...
      await Promise.all(
        [
          ...
          rollbackStagedArtifact(session, staged).catch(() => undefined)
        )
      );
      const latest = await prisma.session.findUnique({
        where: { id: options.sessionId },
    ...
    // Owner paths and all staged ledger rows committed in the same transaction.
    // Only replacement cleanup remains; failures keep the old row charged/orphaned.
    await completeStagedArtifactPublishes(
      session,
      commit.stagedPublications
    ).catch(() => undefined);
    ...
    if (!stagedArtifactsCommitted && stagedArtifacts.length > 0) {
      try {
        const latest = await prisma.session.findUnique({
          where: { id: options.sessionId },
          select: {
        ...
        const ownerReferences = stagedArtifacts.map((staged) => {
          switch (staged.category) {
            case 'recordings':
              return latest?.recordingPath;
            case 'transcripts':
              return latest?.transcriptPath;
            case 'summaries':
        ...
        if (latest?.status === 'COMPLETED' && readback.outcome === 'committed') {
          stagedArtifactsCommitted = true;
          await completeStagedArtifactPublishes(
            session,
            readback.publications
          ).catch(() => undefined);
          if (stagedAudio) {
            await deleteRecordingDraft(session).catch(() => undefined);
          }
          if (stagedTranscriptArtifacts) {
            await deleteTranscriptDraft(session).catch(() => undefined);
          }
          return {
            success: true,
        ...
        if (readback.outcome === 'not_committed') {
          ...
          logger.error(
            { sessionId: options.sessionId },
            '[finalize] publication outcome unknown; preserving staged artifacts'
          );
        }
      } catch (readbackError) {
        logger.error(
          { sessionId: options.sessionId, err: readbackError },
          '[finalize] publication readback failed; preserving staged artifacts'
        );
      }
    }
    ...
    if (error instanceof StoredArtifactQuotaExceededError) {
      throw new FinalizeSessionError(402, {
        error: 'Storage quota exceeded; session artifacts were not published',
        quota: 'storage_bytes',
      });
    }
...
async function resolveTranscriptBundle(
  ...
  let candidate: PersistedTranscriptBundle | null = null;
  ...
  } else if (clientBundle) {
    candidate = clientBundle;
  }

  if (!candidate) return null;
  try {
    // Old draft files can predate request admission. The merged object graph is
    // admitted again immediately before finalize persists it.
    return admitPersistedTranscriptBundle(candidate);
  } catch (error) {
    if (error instanceof SessionTranscriptPayloadError) {
      throw new FinalizeSessionError(error.status, { error: error.message });
    }
    throw error;
  }
}
...
export async function runBackgroundLLMTasks(params: BackgroundTaskParams) {
  ...
  if (!(await isPaymentBenefitAvailable(params.userId))) {
    logger.warn(
      { sessionId: params.sessionId, userId: params.userId },
      '[finalize-bg] payment account frozen, skip paid background LLM tasks'
    );
    return;
  }
  ...
  try {
    ...
    const reportAndTitlePromise = (async () => {
      ...
      if (!allowFinalSummary) {
        ...
        try {
          const reportResult = await generateOrReuseSessionReport({
            ...
            callLLM: (system: string, userMessage: string, execution) =>
              callLLM(system, userMessage, {
                ...finalRouting,
          ...
          logger.info(
            {
              ...
              sourceHash: reportResult.sourceHash,
            },
            reportResult.status === 'in_progress'
              ? '[finalize-bg] same report version already in progress'
              : '[finalize-bg] report generation resolved'
          );
        } catch (error) {
          // 报告预算/供应商失败不应阻断下面的轻量标题生成；service 已负责失败终态。
          logger.error(
            { sessionId: params.sessionId, err: serializeError(error) },
            '[finalize-bg] report generation failed'
          );
        }
      }
